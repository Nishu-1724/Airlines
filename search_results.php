...
$query = "SELECT f.*, 
          ...
          WHERE (LOWER(c1.name) LIKE LOWER(:from) OR LOWER(c1.airport_code) LIKE LOWER(:from))
          AND (LOWER(c2.name) LIKE LOWER(:to) OR LOWER(c2.airport_code) LIKE LOWER(:to))
          AND f.flight_date = :date
          AND f.flight_class = :class
...
<head>
    ...
    <title>Search Results - Indian Airways</title>
    ...
    <div class="container mt-5">
        ...
        <?php if (empty($flights)): ?>
            <div class="alert alert-info">
                No flights found matching your criteria. Please try different dates or cities.
            </div>
        <?php else: ?>
            <div class="row">
